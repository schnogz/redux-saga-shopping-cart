...
nativeObject = YAML.load('db.yml',(database) => {
  ...
  if (!cart) {
    return res.status(500).json({ error:"No cart found with the specified ID", owner })
  }
  
  const item = database.items.find(item => item.id === itemID);
  
  if (!item) {
    return res.status(500).json({ error:"No item found with the specified ID", itemID })
  }
...
app.get("/user/:id",(req,res) => {
  ...
  if (!user) {
    return res.status(500).json({ error:"No user with the specified ID", id })
  } else {
...
app.use(["/cart/validate/:owner","/cart/:owner","/card/charge/:owner"],(req,res,next)=>{
  ...
  if (!cart){
    return res.status(404).json({ error:"No cart with the specified owner", owner });
  }
  else {
    req.cart = cart;
    next();
...
app.get('/tax/:symbol',(req,res) => {
  ...
  if (!taxRate) {
    return res.status(500).json({ symbol, error:"No tax rate info for symbol " + symbol });
  }
  
  res.status(200).json({ rate:taxRate.rate })
});
